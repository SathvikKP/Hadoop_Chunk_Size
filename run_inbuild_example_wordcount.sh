...
for size in "${DATASET_SIZES[@]}"; do
  hadoop fs -rm -r -skipTrash "/output/gutenberg/wordcount_v1_${size}"
done
...
for size in "${DATASET_SIZES[@]}"; do
  ...
  /usr/bin/time -v -o "${SIZE_DIR}/metrics/time.txt" hadoop jar $HADOOP_EXAMPLES_JAR wordcount -D mapreduce.job.name="$JOB_NAME" $INPUT_DIR $OUTPUT_DIR 2>&1 | tee "${SIZE_DIR}/logs/job.log"

  # kill monitoring
  kill $SAR_PID
  sleep 2
  hdfs dfs -get /output/gutenberg/wordcount_v1_${size} ${SIZE_DIR}/outputs/
done
